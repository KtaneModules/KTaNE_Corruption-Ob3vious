...
namespace DelaunatorSharp
{
    public class Delaunator
    {
        ...
        {
            return Array.ConvertAll<int, IPoint>(Hull, (x) => Points[x]);
        }
        ...
        {
            //.Zip doesn't seem to exist in C#4 so this is my awesome alternate that doesn't suck at all -red
            List<IPoint> pointsList = points.ToList();
            List<IPoint> modifiedPoints = points.Skip(1).ToList(); //first part of .Zip
            modifiedPoints.Add(points.FirstOrDefault()); //append

            List<IEdge> edges = new List<IEdge>(); //we were returning edges i think
            for (int i = 0; i < points.Count(); i++) //for each element in points
            {
                int dummy = i;
                edges.Add(new Edge(0, pointsList[dummy], modifiedPoints[dummy])); //this should make edges the same way the original zip did
            }

            return edges; //i really think this is analogous but it's very likely i did something wrong hopefully not though
        }
